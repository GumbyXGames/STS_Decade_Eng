...
import dcd_eng.Power.KamenRideHibikiPower;
...
public class KamenRideHibiki extends AbstractCustomCardWithType {
   ...
   public KamenRideHibiki() {
      super("KamenRideHibiki", NAME, "img/cards/KamenRideHibiki.png", 1, DESCRIPTION, CardType.POWER, AbstractCardEnum.DCD, CardRarity.UNCOMMON, CardTarget.SELF, AbstractCustomCardWithType.CardColorType.Hibiki);
   ...
   public void use(AbstractPlayer p, AbstractMonster m) {
      if (p.hasPower("DecadeViolentEmotionPower")) {
         AbstractDungeon.actionManager.addToBottom(new ApplyPowerAction(p, p, new HibikiKurenaiSpecialPower(p, 5), 5));
      } else {
         if (DCDmod.HenshinTrigger) {
            ...
            CardCrawlGame.sound.playA("hibiki_henshin", 0.0F);
            if (!DCDmod.BGMTrigger) {
               TurnTimer.StopBGM(false);
               CardCrawlGame.music.playTempBGM("hibiki_OP1.ogg");
            }

            AbstractDungeon.actionManager.addToBottom(new ApplyPowerAction(AbstractDungeon.player, AbstractDungeon.player, new StrengthPower(AbstractDungeon.player, 2), 1));
         } else {
            AbstractDungeon.actionManager.addToTop(new HenshinAndRemoveKamenRideAction(p, p, this.KRNumber, "hibiki"));
         }

         AbstractDungeon.actionManager.addToBottom(new ApplyPowerAction(p, p, new KamenRideHibikiPower(p), 1));
      }
