...
public class RemoveKamenRideAction2 extends AbstractGameAction {
   public RemoveKamenRideAction2(AbstractCreature target, AbstractCreature source) {
      this.target = target;
      this.source = source;
   ...
   public void update() {
      if (this.duration == Settings.ACTION_DUR_FAST) {
         if (this.target.hasPower("KamenRideDecadePower")) {
            AbstractDungeon.actionManager.addToTop(new RemoveSpecificPowerAction(this.target, this.source, "KamenRideDecadePower"));
         }
