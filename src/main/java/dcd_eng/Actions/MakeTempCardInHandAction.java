...
public class MakeTempCardInHandAction extends AbstractGameAction {
   ...
   public void update() {
      if (this.amount == 0) {
         ...
         if (this.amount + AbstractDungeon.player.hand.size() > 10 && BaseMod.MAX_HAND_SIZE <= 10) {
   ...
   private void addToHand(int handAmt) {
      AbstractCard c = this.makeNewCard();
      ...
      switch (this.amount) {
         ...
         case 1:
            if (handAmt == 1) {
               if (this.isOtherCardInCenter) {
                  AbstractDungeon.effectList.add(new ShowCardAndAddToHandEffect(c, (float)Settings.WIDTH / 2.0F - (PADDING + AbstractCard.IMG_WIDTH), (float)Settings.HEIGHT / 2.0F));
               } else {
         ...
         case 2:
            if (handAmt == 1) {
               AbstractDungeon.effectList.add(new ShowCardAndAddToHandEffect(c, (float)Settings.WIDTH / 2.0F - (PADDING + AbstractCard.IMG_WIDTH * 0.5F), (float)Settings.HEIGHT / 2.0F));
            } else if (handAmt == 2) {
               AbstractDungeon.effectList.add(new ShowCardAndAddToHandEffect(c, (float)Settings.WIDTH / 2.0F + PADDING + AbstractCard.IMG_WIDTH, (float)Settings.HEIGHT / 2.0F));
               AbstractDungeon.effectList.add(new ShowCardAndAddToHandEffect(c, (float)Settings.WIDTH / 2.0F - (PADDING + AbstractCard.IMG_WIDTH), (float)Settings.HEIGHT / 2.0F));
            }
         ...
         default:
            for(int i = 0; i < handAmt; ++i) {
               AbstractDungeon.effectList.add(new ShowCardAndAddToHandEffect(c, MathUtils.random((float)Settings.WIDTH * 0.2F, (float)Settings.WIDTH * 0.8F), MathUtils.random((float)Settings.HEIGHT * 0.3F, (float)Settings.HEIGHT * 0.7F)));
            }
      }
