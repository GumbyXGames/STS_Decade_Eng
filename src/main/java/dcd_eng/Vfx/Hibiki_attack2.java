...
public class Hibiki_attack2 extends AbstractGameEffect {
   ...
   public Hibiki_attack2(AbstractMonster m, int d, DamageInfo.DamageType damageType, int mn) {
      this.damage = d;
      this.m = m;
      this.magicNumber = mn;
   ...
   public void update() {
      ...
      if (this.duration < 0.9F && this.A1) {
         for(AbstractMonster monster : AbstractDungeon.getMonsters().monsters) {
            if (!monster.isDead && !monster.isDying && monster != this.m) {
      ...
      if (this.duration < 0.2F && this.A2) {
         ...
         AbstractDungeon.actionManager.addToBottom(new ApplyPowerAction(this.m, AbstractDungeon.player, new HibikiBurnPower(this.m, this.magicNumber, AbstractDungeon.player), this.magicNumber));
