   ...
   public void update() {
      if (!DCDmod.AnimationTrigger && AbstractDungeon.player.hasPower("KamenRideFaizPower")) {
         ...
         if (this.duration < 3.5F && this.stage == 0) {
            ...
            AbstractAnimation.changeAnimation(VfxController.far_effect2_A, FaizAnimationAction.faiz_FAR2);
            VfxController.far_effect2_A.state.setAnimation(0, "FAR2", false);
         ...
         if (this.duration < 3.0F && this.stage == 1) {
            assert VfxController.far_effect2_A != null;

            AbstractAnimation.changeAnimation(VfxController.far_effect2_A, FaizAnimationAction.faiz_FAR);
            VfxController.far_effect2_A.state.setAnimation(0, "FAR3", false);
            ++this.stage;
         }

         if (this.duration < 2.5F && this.stage == 2) {
            for(int i = 0; i < 5; ++i) {
         ...
         if (this.duration < 1.5F && this.stage == 3) {
            AbstractDungeon.actionManager.addToBottom(new DamageAction(this.m, new DamageInfo(AbstractDungeon.player, 5, DamageType.NORMAL), AttackEffect.BLUNT_HEAVY));
            ++this.stage;
         }

         if (this.duration < 1.0F && this.stage == 4) {
            AbstractDungeon.actionManager.addToBottom(new DamageAction(this.m, new DamageInfo(AbstractDungeon.player, 5, DamageType.NORMAL), AttackEffect.BLUNT_HEAVY));
            ++this.stage;
         }
