...
public class AbstractHandAnimation {
   public static List<AbstractHandAnimation> H_animations = new ArrayList();
   ...
   public static void changeAnimation(AbstractHandAnimation animation, AnimationLoader loader) {
   ...
   public AbstractHandAnimation(String id, String atlasUrl, String skeletonUrl, float scale, float positionX, float positionY, float hb_w, float hb_h, float hbScale) {
      this.softRef1 = new SoftReference(H_animations);
      this.movable = true;
      this.visible = true;
      this.id = id;
   ...
   public AbstractHandAnimation(String atlasUrl, String skeletonUrl, float scale, float positionX, float positionY, float hb_x, float hb_y, float hb_w, float hb_h) {
      this((String)null, atlasUrl, skeletonUrl, scale, positionX, positionY, hb_w, hb_h, 1.0F);
   }

   public AbstractHandAnimation(String atlasUrl, String skeletonUrl, float scale, float positionX, float positionY, float hb_x, float hb_y) {
      this((String)null, atlasUrl, skeletonUrl, scale, positionX, positionY, 0.0F, 0.0F, 1.0F);
   }

   public AbstractHandAnimation(String imgUrl, float positionX, float positionY, float hb_w, float hb_h, float hbScale) {
      this.softRef1 = new SoftReference(H_animations);
      this.movable = true;
      this.visible = true;
      this.loadImg(imgUrl);
   ...
   public static void addAnimation(AbstractHandAnimation abstractAnimation) {
      if (abstractAnimation != null) {
         for(AbstractHandAnimation animation : H_animations) {
            if (animation.id != null && animation.id.equals(abstractAnimation.id)) {
               return;
            }
         }

         H_animations.add(abstractAnimation);
      }
   }

   public AbstractHandAnimation(String imgUrl, float positionX, float positionY, float hb_w, float hb_h) {
      this(imgUrl, positionX, positionY, 1.0F, hb_w, hb_h);
   }

   public AbstractHandAnimation(String imgUrl, float positionX, float positionY) {
   ...
   public static AbstractHandAnimation getAnimation(String id) {
      if (id == null) {
         return null;
      } else {
         for(AbstractHandAnimation animation : H_animations) {
   ...
   public static AbstractHandAnimation clear(String id) {
      if (id == null) {
         return null;
      } else {
         int x = 0;
